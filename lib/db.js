...
function migrate(db) {
  ...
  // Organizer's decision: the Admin Dashboard has its own PIN (6363); the
  // staff password for every worker station stays 3636. Upgrade an existing
  // DB still on the old shared default.
  if (!process.env.ADMIN_PIN) {
    db.prepare("UPDATE config SET value = '6363' WHERE key = 'admin_pin' AND value = '3636'").run();
  }
}
