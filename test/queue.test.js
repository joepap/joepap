...
test('queue: join, live position, done advances the line, undo restores spot', async () => {
  assert.strictEqual((await post('/api/join', { name: 'Alpha One', token: 'a' })).position, 1);
  assert.strictEqual((await post('/api/join', { name: 'Bravo Two', token: 'b' })).position, 2);
  assert.strictEqual((await post('/api/join', { name: 'Charlie Three', token: 'c' })).position, 3);

  // Same device rejoining updates the name but keeps the spot.
  const rejoin = await post('/api/join', { name: 'Alpha Won', token: 'a' });
  ...
  const bad = await post('/api/join', { name: 'x', token: 'z' });
  assert.ok(bad.error);
});
